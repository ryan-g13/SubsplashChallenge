...
console.log(YTAPI_KEY)
// Loop to retrieve relevant YT fields for each object in the array.
function pingYT() { // subSplashResults.length
  return new Promise ((resolve, reject) => {
    for(let i = 0; i < 20; i++ ) {
      if(subSplashResults[i].youtube_url.slice(-13, -11) !== 'v=') {
        continue;
      }
      var req = https.get(`https://www.googleapis.com/youtube/v3/videos?part=snippet%2C+contentDetails%2C+statistics&id=${subSplashResults[i].youtube_url.slice(-11)}&key=AIzaSyAapZLOdvImXSrChczIAVeAiWR_-y4Q1LY`, function (res) {
        let chunks = [];
        ...
        res.on("end", function () {
          ...
          console.log(subSplashResults[i].duration);
          subSplashResults[i].views = bodyString.items[0].statistics.viewCount;
          return subSplashResults;
        });
        // Object values are being assigned correctly in the above statements but are not persisting to global variable.   
        resolve(subSplashResults);
      });
...
console.log(subSplashResults[0].views)
// -----------------------------------------------------------------------------------------------
/*
...
function sortArray(results) {
  const reachSortedArray = results.sort(compare);
  console.log('Sorted starts', reachSortedArray);
}

// const tableReadyArray = subSplashResults.filter(item => item.views > 100 && (item.duration.charAt(3) === 'H' || item.duration.slice(2,4).parseInt() > 45));

// -----------------------------------------------------------------------------------------------
// Appending elements to the HTML document
// -----------------------------------------------------------------------------------------------
// $(reachSortedArray.title).appendTo('#row1');
// Scaffolding for using jQuery to append things to the DOM 
/* Logic: let rowNum = 1; 
  tableReadyArray.forEach((row) => {
    $(reachSortedArray.title).appendTo(`#row${rowNum}`);
    $(reachSortedArray.duration).appendTo(`#row${rowNum}`);
    $(reachSortedArray.youtube_url).appendTo(`#row${rowNum}`);
    $(reachSortedArray.reach).appendTo(`#row${rowNum}`);
    rowNum++;
})*/
// -----------------------------------------------------------------------------------------------
// Function Calls 
